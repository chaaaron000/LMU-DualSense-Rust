...
- ABS가 없는 차량: TinyPedal Trailing 방식으로 주행 중 보정한 앞/뒤 타이어 유효 반경과 휠 회전속도에서 slip ratio를 계산하고, raw brake가 2%를 넘은 상태에서 wheel lock이 30% 이상이면 pulse 피드백
...
- TC가 개입하지 않아도 TinyPedal Trailing 방식의 wheel slip이 10% 이상이고 raw throttle이 2%를 넘으면 pulse 피드백
...
Wheel lock과 wheel spin 감지 민감도도 설정에서 변경할 수 있습니다.
...
Slip threshold는 비율 값입니다. 예를 들어 `0.30`은 30% wheel lock, `0.10`은 10% wheel spin을 의미합니다. 향후 GUI는 이 설정 필드에 연결할 수 있습니다.
...
ABS가 없는 차량의 wheel lock과 TC가 없는 차량의 wheel slip 계산은 TinyPedal의 Wheels module과 Trailing widget을 기준으로 합니다. 좌우 휠 회전 차이가 0.2% 미만인 구간에서 `차속 ÷ 축 평균 회전속도`로 앞/뒤 타이어 유효 반경을 EMA 보정한 뒤, 네 바퀴 중 가장 큰 제동 또는 가속 slip을 사용합니다. 차속 1 m/s 이하에서는 계산하지 않습니다.
...
```text
INFO [LIVE] G  4 | 184 km/h | RPM  7420/8000  | THR  78% | BRK   0% | ABS OFF | LOCK   0% | TC ON  | SLIP  12% | L2 NORMAL | R2 PULSE(6)
```
